	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			for (int k = 0; k < 3; k++) {
				...
				wGrad[i][j][k] = Eigen::Vector3d(wGrad_HComps[i].x() * wComp[j].y() * wComp[k].z(),
...
void Interpolation::computeWeights(PartWeights& weights, PartWeightGradients& gradients, const Eigen::Vector3d pos)
{
}
